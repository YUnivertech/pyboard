...
class DBManager:
    ...
    def connect( self, _project_name ):
        ...
        if not db_exists:
            self.cursor.execute( "CREATE TABLE IF NOT EXISTS 'Cards'( 'uid' UNSIGNED BIGINT PRIMARY KEY, 'name' TEXT, 'description' TEXT, 'color' TEXT );" )
            self.cursor.execute( "CREATE TABLE IF NOT EXISTS 'Boards'( 'uid' UNSIGNED BIGINT PRIMARY KEY, 'name' TEXT, 'description' TEXT );" )
            self.cursor.execute( "CREATE TABLE IF NOT EXISTS 'Relations'( 'board_uid' UNSIGNED BIGINT, 'tag_name' TEXT, 'tag_value' TEXT, 'card_uid' UNSIGNED BIGINT, UNIQUE( 'board_uid', 'tag_name', 'card_uid' ), FOREIGN KEY( card_uid ) REFERENCES Cards( uid ) ON DELETE CASCADE, FOREIGN KEY( board_uid ) REFERENCES Boards( uid ) ON DELETE CASCADE );" )
    ...
    def add_board( self, _board_uid, _board_name, _board_description ):
        ...
        self.cursor.execute( "INSERT INTO `Boards` ( `uid`, `name`, `description` ) VALUES ( ?, ?, ? );", ( _board_uid, _board_name, _board_description ) )
    ...
    def add_card( self, _card_uid, _card_name, _card_description, _card_color ):
        ...
        self.cursor.execute( "INSERT INTO `Cards` ( `uid`, `name`, `description`, `color` ) VALUES ( ?, ?, ?, ? );", ( _card_uid, _card_name, _card_description, _card_color ) )
        self.cursor.execute( "UPDATE `Info` SET `card_last_used_uid` = ?;", ( _card_uid, ) )
        self.conn.commit( )

    def add_card_to_board( self, _board_uid, _card_uid ):
        self.cursor.execute( "INSERT INTO `Relations` ( `board_uid`, `card_uid` ) VALUES ( ?, ? );", ( _board_uid, _card_uid ) )
        self.conn.commit( )

    def add_card_tag( self, _board_uid, _card_uid, _tag_name, _tag_value ):
        self.cursor.execute( "INSERT INTO `Relations` ( `board_uid`, `card_uid`, `tag_name`, `tag_value` ) VALUES ( ?, ?, ?, ? );", ( _board_uid, _card_uid, _tag_name, _tag_value ) )
        self.conn.commit( )

    def add_card_tag_name( self, _board_uid, _card_uid, _tag_name ):
        self.cursor.execute( "INSERT INTO `Relations` ( `board_uid`, `card_uid`, `tag_name` ) VALUES ( ?, ?, ? );", ( _board_uid, _card_uid, _tag_name ) )
        self.conn.commit( )

    def make_tag_value_permanent( self, _board_uid, _tag_name, _tag_value ):
        self.cursor.execute( "INSERT INTO `Relations` ( `board_uid`, `tag_name`, `tag_value` ) VALUES ( ?, ?, ? );", ( _board_uid, _tag_name, _tag_value ) )
        self.conn.commit( )

    def make_tag_value_temporary( self, _board_uid, _tag_name, _tag_value ):
        self.cursor.execute( "DELETE FROM `Relations` WHERE `board_uid` = ? AND `tag_name` = ? AND `tag_value` = ? AND `card_uid` is NULL;", ( _board_uid, _tag_name, _tag_value ) )
        self.conn.commit( )
    ...
    def update_board( self, _board_uid, _board_name, _board_description ):
        self.cursor.execute( "UPDATE `Boards` SET `name` = ?, `description` = ? WHERE `uid` = ?;", ( _board_name, _board_description, _board_uid ) )
        self.conn.commit( )

    def update_board_name( self, _board_uid, _board_name ):
        self.cursor.execute( "UPDATE `Boards` SET `name` = ? WHERE `uid` = ?;", ( _board_name, _board_uid ) )
        self.conn.commit( )

    def update_board_description( self, _board_uid, _board_description ):
        self.cursor.execute( "UPDATE `Boards` SET `description` = ? WHERE `uid` = ?;", ( _board_description, _board_uid ) )
        self.conn.commit( )

    def update_card( self, _card_uid, _card_name, _card_description, _card_color ):
        self.cursor.execute( "UPDATE `Cards` SET `name` = ?, `description` = ?, `color` = ? WHERE `uid` = ?;", ( _card_name, _card_description, _card_color, _card_uid ) )
        self.conn.commit( )

    def update_card_name( self, _card_uid, _card_name ):
        self.cursor.execute( "UPDATE `Cards` SET `name` = ? WHERE `uid` = ?;", ( _card_name, _card_uid ) )
        self.conn.commit( )

    def update_card_description( self, _card_uid, _card_description ):
        self.cursor.execute( "UPDATE `Cards` SET `description` = ? WHERE `uid` = ?;", ( _card_description, _card_uid) )
        self.conn.commit( )

    def update_card_color( self, _card_uid, _card_color ):
        self.cursor.execute( "UPDATE `Cards` SET `color` = ? WHERE `uid` = ?;", ( _card_color, _card_uid) )
        self.conn.commit( )

    def update_card_tag( self, _board_uid, _card_uid, _new_tag_name, _new_tag_value, _prev_tag_name ):
        self.cursor.execute( "UPDATE `Relations` SET `tag_name` = ?, `tag_value` = ? WHERE `board_uid` = ? AND `tag_name` = ? AND `card_uid` = ?;", ( _new_tag_name, _new_tag_value, _board_uid, _prev_tag_name, _card_uid ) )
        self.conn.commit( )

    def update_card_tag_name( self, _board_uid, _card_uid, _new_tag_name, _prev_tag_name ):
        self.cursor.execute( "UPDATE `Relations` SET `tag_name` = ? WHERE `board_uid` = ? AND `tag_name` = ? AND `card_uid` = ?;", ( _new_tag_name, _board_uid, _prev_tag_name, _card_uid ) )
        self.conn.commit( )

    def update_card_tag_value( self, _board_uid, _card_uid, _tag_name, _tag_value ):
        self.cursor.execute( "UPDATE `Relations` SET `tag_value` = ? WHERE `board_uid` = ? AND `tag_name` = ? AND `card_uid` = ?;", ( _tag_value, _board_uid, _tag_name, _card_uid ) )
        self.conn.commit( )

    def update_tag_name( self, _board_uid, _new_tag_name, _prev_tag_name ):
        self.cursor.execute( "UPDATE `Relations` SET `tag_name` = ? WHERE `board_uid` = ? AND `tag_name` = ?;", ( _new_tag_name, _board_uid, _prev_tag_name ) )
        self.conn.commit( )

    def update_tag_value( self, _board_uid, _new_tag_value, _prev_tag_value, _tag_name ):
        self.cursor.execute( "UPDATE `Relations` SET `tag_value` = ? WHERE `board_uid` = ? AND `tag_name` = ? AND `tag_value` = ?;", ( _new_tag_value, _board_uid, _tag_name, _prev_tag_value ) )
        self.conn.commit( )
    ...
    def delete_card_tag( self, _board_uid, _card_uid, _tag_name ):
        self.cursor.execute( "DELETE FROM `Relations` WHERE `board_uid` = ? AND `card_uid` = ? AND `tag_name` = ?;", ( _board_uid, _card_uid, _tag_name ) )
        self.conn.commit( )

    def delete_board_tag( self, _board_uid, _tag_name ):
        self.cursor.execute( "DELETE FROM `Relations` WHERE `board_uid` = ? AND `tag_name` = ?;", ( _board_uid, _tag_name ) )
        self.conn.commit( )

    def delete_tag_value( self, _board_uid, _tag_name, _tag_value ):
        self.cursor.execute( "DELETE FROM `Relations` WHERE `board_uid` = ? AND `tag_name` = ? AND `tag_value` = ?;", ( _board_uid, _tag_name, _tag_value ) )
        self.conn.commit( )

    def remove_card_from_board( self, _board_uid, _card_uid ):
        self.cursor.execute( "DELETE FROM `Relations` WHERE `board_uid` = ? AND `card_uid` = ?;", ( _board_uid, _card_uid ) )
        self.conn.commit( )
    ...
    def get_board_grouped_cards( self, _board_uid, _tag_name ):
        self.cursor.execute( "SELECT DISTINCT `tag_value` FROM `Relations` WHERE `board_uid` = ? AND `tag_name` = ?;", ( _board_uid, _tag_name ) )
        fetched_data  = self.cursor.fetchall( )
        grouped_cards = { }
        for i in fetched_data:
            self.cursor.execute( "SELECT `Cards`.`uid`, `Cards`.`name`, `Cards`.`description`, `Cards`.`color` FROM `Cards`, `Relations` WHERE `Cards`.`uid` = `Relations`.`card_uid` AND `Relations`.`board_uid` = ? AND `Relations`.`tag_name` = ? AND `Relations`.`tag_value` = ?;", ( _board_uid, _tag_name, i[ 0 ] ) )
    ...
    def get_board( self, _board_uid ):
        self.cursor.execute( "SELECT `name`, `description` FROM `Boards` WHERE `uid` = ?;", ( _board_uid, ) )
        fetched_data = self.cursor.fetchall( )
    ...
    def get_board_name( self, _board_uid ):
        self.cursor.execute( "SELECT `name` FROM `Boards` WHERE `uid` = ?;", ( _board_uid, ) )
        fetched_data = self.cursor.fetchall( )
    ...
    def get_board_description( self, _board_uid ):
        self.cursor.execute( "SELECT `description` FROM `Boards` WHERE `uid` = ?;", ( _board_uid, ) )
        fetched_data      = self.cursor.fetchall( )
    ...
    def get_board_tag_names( self, _board_uid ):
        self.cursor.execute( "SELECT DISTINCT `tag_name` FROM `Relations` WHERE `board_uid` = ?;", ( _board_uid, ) )
        fetched_data = self.cursor.fetchall( )
        tag_names    = [ ]
        for i in fetched_data:
    ...
    def get_card( self, _card_uid ):
        self.cursor.execute( "SELECT `name`, `description`, `color` FROM `Cards` WHERE `uid` = ?;", ( _card_uid, ) )
        fetched_data = self.cursor.fetchall( )
        card         = fetched_data[ 0 ]
    ...
    def get_card_name( self, _card_uid ):
        self.cursor.execute( "SELECT `name`, FROM `Cards` WHERE `uid` = ?;", ( _card_uid, ) )
        fetched_data = self.cursor.fetchall( )
    ...
    def get_card_description( self, _card_uid ):
        self.cursor.execute( "SELECT `description` FROM `Cards` WHERE `uid` = ?;", ( _card_uid, ) )
        fetched_data     = self.cursor.fetchall( )
    ...
    def get_card_color( self, _card_uid ):
        self.cursor.execute( "SELECT `color` FROM `Cards` WHERE `uid` = ?;", ( _card_uid, ) )
        fetched_data = self.cursor.fetchall( )
    ...
    def get_cards_uid_in_board( self, _board_uid ):
        self.cursor.execute( "SELECT DISTINCT `card_uid` FROM `Relations` WHERE `board_uid` = ? AND `tag_name` is NULL;", ( _board_uid, ) )
        fetched_data = self.cursor.fetchall( )
        cards_uid    = [ ]
        for i in fetched_data:
    ...
    def get_cards_in_board( self, _board_uid ):
        self.cursor.execute( "SELECT `Cards`.`uid`, `Cards`.`name`, `Cards`.`description`, `Cards`.`color` FROM `Cards`, `Relations` WHERE `Cards`.`uid` = `Relations`.`card_uid` AND `Relations`.`board_uid` = ?;", ( _board_uid, ) )
        fetched_data   = self.cursor.fetchall( )
    ...
    def get_card_tags( self, _board_uid, _card_uid ):
        self.cursor.execute( "SELECT `tag_name`, `tag_value` FROM `Relations` WHERE `board_uid` = ? And `card_uid` = ?;", ( _board_uid, _card_uid ) )
        fetched_data = self.cursor.fetchall( )
        tags         = fetched_data
